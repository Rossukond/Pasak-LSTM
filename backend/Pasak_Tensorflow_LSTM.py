...
def load_data(file_obj_or_path, train_year_range, test_year_range):
    ...
    # # กรองข้อมูลเฉพาะเดือนสิงหาคม, กันยายน, ตุลาคม
...
# Callback เพื่อคำนวณ RMSE ของชุดข้อมูล test หลังจากแต่ละ epoch
class CustomCallback(Callback):
    def __init__(self, X_train, Y_train, X_test, Y_test, scaler_inflow, day):
        ...
        self.train_rmse = []
        self.test_rmse = []
        self.epoch_rmse_data = []  # เพิ่มตัวแปรเก็บข้อมูล RMSE ในแต่ละ epoch
        self.day = day

    def on_epoch_end(self, epoch, logs=None):
        # คำนวณ RMSE สำหรับชุดข้อมูล train
        ...
        # คำนวณ RMSE สำหรับชุดข้อมูล test
        ...
        # อัปเดตข้อมูลเข้า globals() ตามวัน
        globals()[f'epoch_rmse_data_day{self.day}'] = self.epoch_rmse_data

        # แสดงผล RMSE ของ train และ test หลังจากแต่ละ epoch
        print(f'Epoch {epoch + 1}: Train RMSE = {train_rmse}, Test RMSE = {test_rmse}')
        print(f"🔍 ขนาดข้อมูลล่าสุด: {len(self.epoch_rmse_data)} entries")


# ฟังก์ชันแสดงกราฟ RMSE ต่อ Epoch
...
def plot_rmse_vs_epoch(train_rmse, test_rmse, day, window_size=10):
    ...
    # ตรวจสอบว่า window_size ไม่ใหญ่กว่าขนาดข้อมูล
    if window_size > len(train_rmse):
        window_size = len(train_rmse)  # ป้องกัน Error
    ...
    # ฟังก์ชันแสดงกราฟเปรียบเทียบ Actual vs Predicted
...
# ฟังก์ชันคำนวณค่า r, RMSE และ NSE
...
# การแสดงกราฟที่เก็บไว้ใน all_graphs
def show_saved_graphs():
    for graph_name, graph in all_graphs.items():
        print(f"Showing graph: {graph_name}")
        plt.figure(graph.number)  # เลือกกราฟที่เก็บไว้
        plt.show()  # แสดงกราฟ

# สร้าง Dictionary เพื่อเก็บผลลัพธ์ของแต่ละวัน
all_results_train = {}  # เก็บข้อมูล Train
all_results_test = {}   # เก็บข้อมูล Test        

# ฟังก์ชันหลักในการพยากรณ์หลายวัน
def forecast_multiple_days(df_train, df_test, feature_columns, forecast_days,epochs_per_day):
    ...
    for day in range(1, forecast_days + 1):
        ...
        # Custom callback สำหรับเก็บค่า RMSE ของ test
        ...
        # 🔥 เก็บข้อมูล Train และ Test ของแต่ละวัน
        ...
        print(f"📊 Metrics for Day {day} Forecast:")
